...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # angle = 2 * np.pi / self.total_frames * 0
        ...
        # dj: change pose
        # dst_poses[1*3+2] =  1.75 # dj: left hip  rotate +1/pi*180 = +57 deg
        # dst_poses[2*3+2] =  -0.5 # dj: right hip  rotate +1/pi*180 = +57 deg
        # dst_poses[16*3+2] =  1.0 # dj: left shoulder  rotate +1/pi*180 = +57 deg
        # dst_poses[17*3+2] = -1.0 # dj: right shoulder rotate -1/pi*180 = -57 deg
        ...
        if 'motion_bases' in self.keyfilter:
            dst_Rs, dst_Ts = body_pose_to_body_RTs(dst_poses, dst_skel_joints) # dj
            cnl_gtfms = get_canonical_global_tfms(self.canonical_joints)
            results.update({
                'dst_Rs': dst_Rs,
                'dst_Ts': dst_Ts,
                'cnl_gtfms': cnl_gtfms,
                'dst_poses': dst_poses,            # dj: for employinh motionCLIP in run.py
                'dst_skel_joints': dst_skel_joints # dj: for employinh motionCLIP in run.py
            })                                    
